...
import { modifier_bow_armed_buff } from "./bow_armed_buff";

@registerAbility()
export class cast_bow_armed extends fabao_ability
{    
    Spawn(): void {
        if(!IsServer()) return;
    ...
    OnProjectileHit_ExtraData(target: CDOTA_BaseNPC, location: Vector, extraData: any): boolean | void {
        if(target && (target.GetTeam() == this.GetCaster().GetTeam())) {
            target.AddNewModifier(this.GetCaster(), this, modifier_bow_armed_buff.name, {})
            return true;
        }
    }
}